...
module.exports.run = async (client, message, args) => {///oziden aldım helal edin beyler :D
...
let member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.author);
  ...
  .setColor('2f3136')).then(x => x.delete({timeout: 5000}));
    ...
    let et = message.guild.members.cache.filter(member => !member.roles.cache.has(ayarlar.cekilis) || !member.roles.cache.has(ayarlar.etkinlik)).size;
...
${taglilar.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")}`)
    ...
    message.guild.members.cache.filter(s => s.user.username.includes(ayarlar.tag) && !s.roles.cache.has(ekipRolu)).map(x=> x.roles.add(ayarlar.tagrol))                
    }
...
${ozicim.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")} `)
    ...
    message.guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== message.guild.id).size == 0).map(x=> x.roles.add(ayarlar.kayıtsızrol))
